...
public class PasswordToKeyUtils {
    ...
    public static String shaCheck(String strSrc) {
        if (MyApplication.getDaoInstant().getAccountBeanDao().loadAll().get(0).getSha_pwd() == null) {
            return null;
        }
        String salt = MyApplication.getDaoInstant().getAccountBeanDao().loadAll().get(0).getSha_pwd().substring(0, 32);//盐
